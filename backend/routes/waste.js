...
router.get('/', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: result.rows.map(r => ({ id: r.id, materialType: r.material_type, description: r.description, quantity: parseFloat(r.quantity), unit: r.unit, location: r.location, availableFrom: r.available_from, status: r.status, providerName: r.provider_name, industryType: r.industry_type, createdAt: r.created_at })) });
...
router.get('/search', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: result.rows.map(r => ({ id: r.id, materialType: r.material_type, description: r.description, quantity: parseFloat(r.quantity), unit: r.unit, location: r.location, status: r.status, providerName: r.provider_name })) });
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO waste_listings (industry_id, material_type, description, quantity, unit, location, available_from)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
      [req.user.id, materialType, description, quantity, unit, location, availableFrom]
    );
    const r = result.rows[0];
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE waste_listings SET material_type=COALESCE($1,material_type), description=COALESCE($2,description),
       ...
       available_from=COALESCE($6,available_from), status=COALESCE($7,status), updated_at=CURRENT_TIMESTAMP
       WHERE id=$8 RETURNING *`,
      [materialType, description, quantity, unit, location, availableFrom, status, id]
    );
    const r = result.rows[0];
    res.json({ success: true, data: { id: r.id, materialType: r.material_type, quantity: parseFloat(r.quantity), unit: r.unit, status: r.status } });
